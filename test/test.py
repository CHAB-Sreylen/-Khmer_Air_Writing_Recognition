...
def is_saving_state(hand_landmarks, pinch_threshold=0.7):
    thumb_tip = hand_landmarks.landmark[THUMB_TIP]
    ring_tip = hand_landmarks.landmark[RING_TIP]
    thumb_pip = hand_landmarks.landmark[3]  # Thumb's PIP joint
    thumb_len = distance_between_lms(thumb_tip, thumb_pip)
    pinch_len = distance_between_lms(thumb_tip, ring_tip)
    return pinch_len / thumb_len < pinch_threshold
...
def draw_center_box(image, overlay_img1, overlay_img2, khmer_text, font_khmer, text_position, text_color, color=(25, 167, 206), thickness=2):
    ...
    # Convert the image to PIL for drawing text
    img_pil = Image.fromarray(image_with_box)
    draw = ImageDraw.Draw(img_pil)

    # Draw Khmer text inside the text box
    # text_box_center = (text_box_top_left[0] + (text_box_bottom_right[0] - text_box_top_left[0]) // 2,
    #                    text_box_top_left[1] + (text_box_bottom_right[1] - text_box_top_left[1]) // 2)
    
    # draw.text((text_box_center[0] - 70, text_box_center[1] - 50), khmer_text, font=font_khmer, fill=text_color)
...
#             if stroke:  # Only write non-empty strokes
#                 sequence_data = [f'{x:.4f},{y:.4f}' for x, y in stroke]
#                 sequence_str = ' '.join(sequence_data)  # Join all points in the stroke into a single string
#                 writer.writerow([f'Stroke {i+1}', sequence_str])
#             else:
#                 writer.writerow([f'Stroke {i+1}', ''])
def save_strokes_csv(strokes, filename):
    combined_stroke = []
    for stroke in strokes:
    ...
    with open(file_path, 'w', newline='') as file:
        writer = csv.writer(file)
        if combined_stroke:  # Only write if there is any stroke data
            sequence_data = [f'{x:.4f},{y:.4f}' for x, y in combined_stroke]
            sequence_str = ' '.join(sequence_data)  # Join all points into a single string
            writer.writerow(['Combined Stroke', sequence_str])
        else:
            writer.writerow(['Combined Stroke', ''])
...
def show_frame():
    ...
    # Resize the entire processed frame to 200%
