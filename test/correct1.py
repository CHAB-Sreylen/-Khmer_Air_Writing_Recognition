...
def save_strokes_txt(strokes, filename):
    print(f"Saving {len(strokes)} strokes to {filename}")  # Debug print
    with open(filename, 'w') as file:
        for i, stroke in enumerate(strokes):
            stroke_str = ' '.join(f'{x:.4f},{y:.4f}' for x, y in stroke)
            file.write(f'Stroke {i+1}: {stroke_str}\n')
        ...
        if is_drawing_state(hand_landmarks):
            ...
            if not cleared:
                if is_clearing_state(hand_landmarks):
                    strokes.clear()
                    cleared = True
                elif is_saving_state(hand_landmarks):
                    if strokes: 
                        save_strokes_txt(strokes,"saved_stroke.txt")
                        print("Strokes saved.")
                        # Uncomment the following line if you want to clear the strokes after saving
                        # strokes.clear()
                        # Optionally set cleared to True if you want to avoid multiple saves on a single gesture
                        # cleared = True
