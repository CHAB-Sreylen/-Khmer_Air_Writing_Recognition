...
def chaikins_algorithm(points, iterations=2):
    ...
    for iteration in range(iterations):
        ...
        # Debug: print the current points before the iteration
        print(f"Iteration {iteration+1} - before smoothing: {points}")
        ...
        for i in range(len(points) - 1):
            ...
            # Add the first point if it's the first iteration
            if i == 0:
                new_points.append(tuple(p0))  # Preserve the first point

            # Add the new points
            new_points.append(tuple(q))
            new_points.append(tuple(r))

            # Add the last point if it's the last iteration
            if i == len(points) - 2:
                new_points.append(tuple(p1))  # Preserve the last point

        # Update points for the next iteration
        points = new_points

        # Debug: print the new points after the iteration
        print(f"Iteration {iteration+1} - after smoothing: {points}")
...
def save_strokes_txt(strokes,filename):
    with open(filename, 'w') as file:
        for i, stroke in enumerate(strokes):
            # Convert each point in the stroke to a string and join with commas
...
def show_frame():
    ...
    scale_percent = 200  # Percent of original size, increase for larger scale
